...
\caption[Example geometry and voxelized representation for a Poisson problem]{Left: Example of the geometry for a Poisson problem. Right: Our voxelized representation of this computational domain.}
...
\caption[Illustration of coarsening strategy.]{Illustration of our coarsening strategy. Note the geometric and topological discrepancy between different resolution levels.}
...
\paragraph{Instability:} When using multigrid as the solver instead of a preconditioner, the iterates produced by the sequence of V-Cycles can become highly oscillatory or divergent unless a substantial smoothing effort is spent near the
...
\paragraph{Compact representation of $\mathcal{L}$:} The discrete operator $\mathcal{L}$ contains the standard 7-point stencil in the interior of the domain, with a
...
\paragraph{Using zero initial guess and boundary conditions:} When the multigrid V-Cycle is used as a preconditioner, it is solving a Poisson problem with zero boundary conditions,
...
\begin{figure}[t]
\center{
\includegraphics[width=.32\columnwidth]{poisson/figures/smoke_192_closeup_00090.png} 
\includegraphics[width=.32\columnwidth]{poisson/figures/smoke_384_closeup_00090.png}
\includegraphics[width=.32\columnwidth]{poisson/figures/smoke_768_closeup_00090.png}
}
\caption[Smoke plume simulation at various resolutions]{Smoke plume simulation at resolutions of $192^2\times 288$ (left), $384^2\times 576$ (middle) and $768^2\times 1152$ (right).}
\label{fig_smoke_sphere}
\end{figure}


\begin{figure}[t]
\center{
\includegraphics[width=.45\columnwidth]{poisson/figures/dragon_no_pool_closeup_00013.png}
\includegraphics[width=.45\columnwidth]{poisson/figures/dragon_no_pool_closeup_00027.png}
}
\center{
\includegraphics[width=.45\columnwidth]{poisson/figures/dragon_no_pool_closeup_00036.png}
\includegraphics[width=.45\columnwidth]{poisson/figures/dragon_deep_pool_00084.png}
}
\caption{Free surface water simulation on a $512^3$ grid.}
\label{fig_water}
\end{figure}

\begin{figure}[ht]
\center{
 \includegraphics[width=.45\textwidth]{poisson/figures/smoke_768_closeup_00144.png} 
 \includegraphics[width=.45\textwidth]{poisson/figures/smoke_car_768_00223.png}
}
...
\begin{table}[tb]
 ...
 \caption[Runtime comparison with CG and ICPCG.]{Number of iterations and \textbf{serial} runtimes for our modified conjugate gradient(*) and a ``black box''($\dagger$) solver (top), incomplete Cholesky PCG (middle), and our method (bottom).  Initialization time includes building any matrices and computing incomplete Cholesky factorization for ICPCG or building multigrid hierarchy for MGPCG.}\label{fig_runtime_comparison}
\end{table}


\begin{table}[tb]
\center{\includegraphics[width=.7\columnwidth]{poisson/figures/runtimes1.pdf}}
\caption[Execution cost and parallel speedup for our method.]{Execution cost and parallel speedup for our method. All parallel computations were carried out on a 16-core Intel Xeon X7350 server with 32GB of RAM. (*)Initialization refers to multigrid hierarchy initialization.}
\label{fig_runtimes}
\end{table}
...
\caption[Convergence comparison with CG and ICPCG.]{
